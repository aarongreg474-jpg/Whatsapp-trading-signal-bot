...
# =================================================================
# MARKET STRUCTURE / BREAK OF STRUCTURE (price-action strategy)
# =================================================================

def find_swing_points(df: pd.DataFrame, window: int = 3, lookback: int = 60):
    """
    Returns ordered lists of (position_index, price) for swing highs and lows
    within the recent lookback window, using a simple fractal method (a candle
    is a swing high/low if it's the max/min within `window` candles either side).
    """
    recent = df.iloc[-lookback:]
    highs, lows = [], []
    for i in range(window, len(recent) - window):
...
def signal_market_structure(df: pd.DataFrame, window: int = 3, lookback: int = 60,
                             proximity_pct: float = 0.0025) -> int:
    """
    Mechanical version of a 'break of structure' price-action strategy:
    1. Classify recent structure as bullish (higher highs + higher lows),
       bearish (lower highs + lower lows), or mixed/sideways.
    2. Check if the most recent close has broken beyond the last swing
       high (bullish break of structure) or swing low (bearish).
    3. Also fires if price is currently retesting that broken level (a
       "key area" retest), which is when this strategy looks for a
    ...
    if bullish_structure:
        ...
        near_key_area = abs(price - last_high) / price < proximity_pct
        if broke_up or near_key_area:
            return 1
    elif bearish_structure:
        broke_down = price < last_low
        near_key_area = abs(price - last_low) / price < proximity_pct
        if broke_down or near_key_area:
            return -1
