...
def fetch_candles(symbol: str, interval: str, output_size: int = CANDLE_LOOKBACK) -> pd.DataFrame:
    ...
    # Forex pairs from Twelve Data often omit volume — fall back to a
    # synthetic proxy (tick range) so volume-based indicators don't crash.
    ...
    # Drop the most recent candle if it hasn't fully closed yet — analyzing
    # a still-forming candle can produce misleading indicator values that
    # change again once the candle actually completes.
