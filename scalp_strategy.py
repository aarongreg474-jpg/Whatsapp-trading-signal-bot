...
This runs independently of the price-action (Ivy Trader) strategy in
signal_engine.py — either one firing sends its own Discord alert.
"""
...
def _bars_since_sign_change(sign_series: pd.Series, max_lookback: int = 10):
    """How many bars ago the sign last flipped (0 = flipped on the most recent
    candle), or None if no flip found within max_lookback."""
    n = len(sign_series)
...
def evaluate_scalp_signal(df: pd.DataFrame) -> dict:
    ...
    # 1. Moving average cross (3 EMA vs 10 EMA)
    ...
    # 3. MACD momentum (custom periods: 15, 27, 9)
    ...
    # 5. Candle color check — reject if a strong candle opposes the trade
